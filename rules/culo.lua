...
game.down_func={
    ...
    K_ESCAPE =   {"self.actions.exit()","local"}, 
    K_F5     =   {"self.actions.show()","local"}, 
    K_UP     =   {"self.actions.showalt()","local"}, 
    K_z      =   {"self.actions.clear_selection","global"}, 
    K_t      =   {"self.actions.throw_cards","global"}, 
    K_RETURN =   {"self.actions.throw_cards","global"}, 
    K_e      =   {"self.actions.end_turn()","global"}, 
    K_s      =   {"self.actions.sort('points')","local"}, 
    K_d      =   {"self.actions.draw_a_card","global"}, 
    K_p      =   {"self.actions.pass_turn","global"}, 
...
function game:throw_cards(self,selection)
    ending_turn()
    print (".................")
    print (self)
    print (".................")
...
function game:throwable_selection(self,selection)
    ...
    --si ya hay tiradas entonces tendran que comprobarse mas parametros 
        ...
        if len(selection)==len(last_throw) then
            if selection[0].points>last_throw[0].points then
                return true
            end
        end
        return false
